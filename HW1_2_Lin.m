...
%
% psID: 1192656
%% Explore theoretical waiting time for 
% clean workspace
clear; clc
...
% plot expected waiting time    
    ...
    text(3, 1, ...
        {'Expected time for populaton to go infinity',['        $$\sum_{n=1}^{\infty} \frac{1}{\lambda_n} = $$' , num2str(t_exp_infty)]}, ...
        'Interpreter','latex',...
        'fontsize', 14);
 %% simulation
 tlim = 1.5;
 ...
 mysubplot(2,1,1)
  ...
  for i =1:20
        ...
        legend('tlim', 'simulation 1', 'simulation 2', '...')
        vline(tlim);% replot so it's on top
        ylabel('state(n)'); title('"Explosive" pure birth process')
             axis([ 0 2 0 2000])
  mysubplot(2,1,2)
  rng(1); % set seed; so this will be identical simulations
   for i =1:20 
        ...
        axis([ 0 2 0 25])
 vline(tlim)
  ylabel('state(n)'); xlabel('time'); title('close-up look')
